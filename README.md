...
- **Inbox (Входящие):** вместо колокольчика в topbar; уведомления по задачам/дедлайнам/инвайтам, действия `Read/Unread/Archive`, автообновление
- **Activity (admin only):** в корпоративном пространстве есть вкладка журнала действий: создание/назначение/обновление/выполнение задач, присоединение участников
- **Backend logging middleware:** логирует API-запросы (method, path, status, duration, user_id)
